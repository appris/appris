...
#sub get_seq_aln_features {
#	my ($self) = shift;
#	my ($operation) = shift;
#	my ($seq_string) = shift;
#	my ($res_string) = shift;
#	my ($ids) = shift if (@_);
#	my ($format) = shift if (@_);
#	my ($result) = undef;
#	my ($view_panel, $view_sequences) = ('','');
#
#	# get residues report by transcript id
#	my ($res_json);
#	my ($residues, $methods);
#	my ($json) = new JSON;
#	if ( defined $res_string and ($res_string ne '') ) {
#		$res_json = $json->decode($res_string);
#		foreach my $res_rep (@{$res_json}) {
#			my ($r_id) = $res_rep->{'id'};
#			my ($r_methods) = $res_rep->{'methods'};
#			my ($r_list);
#			foreach my $r_method (@{$r_methods}) {
#				my ($m_id) = $r_method->{'id'};
#				my ($m_label) = $r_method->{'label'};
#				my ($m_residues) = $r_method->{'residues'};
#				foreach my $r_residue (@{$m_residues}) {
#					my ($start) = $r_residue->{'start'};
#					my ($end) = $r_residue->{'start'};
#					$end = $r_residue->{'end'} if ( exists $r_residue->{'end'} );
#					my ($annot) = $r_residue->{'annot'} if ( exists $r_residue->{'annot'} );
#					my ($damaged) = $r_residue->{'damaged'} if ( exists $r_residue->{'damaged'} );
#					
...
#					}
#				}
#				$methods->{$m_id} = $m_label unless ( exists $methods->{$m_id} );
#			}
#			$residues->{$r_id} = $r_list if ( defined $r_list );
#		}
#	}
#
##	if ( defined $res_string and ($res_string ne '') ) {
##		$res_json = $json->decode($res_string);
##		foreach my $res_rep (@{$res_json}) {
##			while ( my ($r_id,$r_rep) = each(%{$res_rep}) ) {
##				my ($r_list, $a_list);
##				while ( my ($met,$res_list) = each(%{$r_rep}) ) {
##					foreach my $r (@{$res_list}) {
##						my ($start) = $r->{'start'};
##						my ($end) = $r->{'end'};
##						if ( exists $r->{'title'} and exists $r->{'annot'} ) {
##							my ($title) = $r->{'title'};
##							my ($annot) = $r->{'annot'};
###							push(@{$a_list->{$start}->{'start'}}, {
###								'method'	=> $met,
###								'pos'		=> $start,
###								'title'		=> $title,
###								'annot'		=> $annot
...
##					}
##				}
##				$annots->{$r_id} = $a_list if ( defined $a_list );
##				$residues->{$r_id} = $r_list if ( defined $r_list );
##			}
##		}
##	}
#	
##print STDERR "RESIDUES: \n".Dumper($residues)."\n";
##print STDERR "JSON_RESIDUES: \n".Dumper($res_json)."\n";
##print STDERR "ANNOTS: \n".Dumper($annots)."\n";
##print STDERR "RESIDUES: \n".$res_string."\n";
#
#
...
#		my ($MAX_PEPTIDES) = 70;
#		#my ($NUM_CHAR_TITLE) = 30;
#		my ($NUM_CHAR_TITLE) = 18;
#		
#		if ( $operation eq 'sequences' ) {
#			($view_panel, $view_sequences) = $self->get_seq_features($seq_string, $residues, $MAX_PEPTIDES);
#		}
#		elsif ( $operation eq 'align' ) {
#			($view_panel, $view_sequences) = $self->get_aln_features($seq_string, $residues, $MAX_PEPTIDES, $NUM_CHAR_TITLE);
#		}
#
#		if ( ($view_panel ne '') and ($view_sequences ne '') ) {
#			my ($view_methods) = get_seq_aln_legend($methods);
#			$result = '<html>'.
#						$HTML_TEMPLATE.
#						'<script>var oResidues = '.$res_string.'</script>'.
#						'<body>'.
#						'<div class="seq-browser pull-right">'.
#							'<div class="seq-browser-legend">'.$view_methods.'</div>'.
#							'<div class="seq-browser-panel">'.$view_panel.'</div>'.							
#						'</div>'.
#						'<div class="seq-browser pull-left">'.
#							'<div class="seq-browser-seqs">'.$view_sequences.'</div>'.
#						'</div>'.
#						'</body>'.
#						$HTML_TEMPLATE2.
#					  '</html>';		
...
#	return $result;
#	
#} # end get_seq_aln_features

#sub get_seq_aln_legend($) {
#	my ($methods) = @_;
#	my ($result) = '';
#
#	$result .= '<div class="btn-group-vertical">';
#	if ( exists $methods->{'functional_residue'} ) {
#		my ($label) = $methods->{'functional_residue'};
#		$result .= '<div class="seq-browser-label f"><span class="glyphicon glyphicon-check"></span> <label>'.$label.'</label></div>';		
#	}
#	if ( exists $methods->{'homologous_structure'} ) {
#		my ($label) = $methods->{'homologous_structure'};
#		$result .= '<div class="seq-browser-label m"><span class="glyphicon glyphicon-check"></span> <label>'.$label.'</label></div>';
#	}
#	if ( exists $methods->{'vertebrate_conservation'} ) {
#		my ($label) = $methods->{'vertebrate_conservation'};
#		$result .= '<div class="seq-browser-label c"><span class="glyphicon glyphicon-check"></span> <label>'.$label.'</label></div>';
#	}
#	if ( exists $methods->{'functional_domain'} ) {
#		my ($label) = $methods->{'functional_domain'};
#		$result .= '<div class="seq-browser-label s"><span class="glyphicon glyphicon-check"></span> <label>'.$label.'</label></div>';
#	}
#	if ( exists $methods->{'transmembrane_signal'} ) {
#		my ($label) = $methods->{'transmembrane_signal'};
#		$result .= '<div class="seq-browser-label t"><span class="glyphicon glyphicon-check"></span> <label>'.$label.'</label></div>';
#	}
#	if ( exists $methods->{'principal_isoform'} ) {
#		my ($label) = $methods->{'principal_isoform'};
#		$result .= '<div class="seq-browser-label a"><span class="glyphicon glyphicon-check"></span> <label>'.$label.'</label></div>';
#	}
#	$result .= '</div>';
#	
#	return $result;
#	
#} # end get_seq_aln_legend
...
<link rel="stylesheet" href="http://apprisws.bioinfo.cnio.es/css/jquery.webui-popover.css">

<link rel="stylesheet" href="http://appris.bioinfo.cnio.es/css/bootstrap.min.css">
<script src="http://apprisws.bioinfo.cnio.es/js/jquery-1.8.3.min.js"></script>

<script src="http://apprisws.bioinfo.cnio.es/js/jquery.webui-popover.js"></script>


<script>
var sLigandURL = 'http://www.rcsb.org/pdb/ligand/ligandsummary.do?hetId=';
var sPDBURL = 'http://www.rcsb.org/pdb/explore/explore.do?structureId=';
var sPfamURL = 'http://pfam.xfam.org/family/';

function printAnnotSeqViewer(sid,methods,residue) {
	var annot = '';
	for (var i = 0; i < oResidues.length; i++) {
		var oSeq = oResidues[i];
		if ( oSeq.id === sid ) {
			var met = '';
			for (var j = 0; j < oSeq.methods.length; j++) {
				var oMet = oSeq.methods[j];
				if ( methods.indexOf(oMet.id) !== -1 ) {
					var res = '';
					for (var k = 0; k < oMet.residues.length; k++) {
						var oRes = oMet.residues[k];
						
						var sAnnot = '-';
						if ( typeof(oRes.annot) !== 'undefined' ) { sAnnot = oRes.annot }
													
						if ( ( typeof(oRes.start) !== 'undefined') && ( typeof(oRes.end) !== 'undefined') ) {
							if ( (parseInt(residue) >= parseInt(oRes.start)) && (parseInt(residue) <= parseInt(oRes.end)) ) {
								var filterPattern = /([^ ]*) \\(([^\\)]*)\\)/;
								var sAnnotMatch = oRes.annot.match(filterPattern);
								var sMatchAcc = sAnnotMatch[1];
								var sMatchEval = sAnnotMatch[2];
								if ( (typeof(sMatchAcc) !== 'undefined') && (typeof(sMatchEval) !== 'undefined') ) {
									if ( oMet.id === "homologous_structure") {
										var sMatchId = sMatchAcc.replace(/\_[a-zA-z]*/g,'');
										sAnnot = "<a href='"+sPDBURL+sMatchId+"' target='_blank'>"+sMatchAcc+"</a>" + " (" + sMatchEval + ")";
									}
									else if ( oMet.id === "functional_domain") {
										var sMatchId = sMatchAcc;
										sAnnot = "<a href='"+sPfamURL+sMatchId+"' target='_blank'>"+sMatchAcc+"</a>" + " (" + sMatchEval + ")";
									}
									else {
										sAnnot = sMatchAcc + " (" + sMatchEval + ")";
									}										
								}
								if ( typeof(oRes.damaged) !== 'undefined' && oRes.damaged === 1 ) {
									res += "<tr><td>"+oRes.start+"</td><td>"+oRes.end+"</td><td>"+sAnnot+" - damaged</td>";
								}
								else {
									res += "<tr><td>"+oRes.start+"</td><td>"+oRes.end+"</td><td>"+sAnnot+"</td>";
								}
							}
						}
						else if ( typeof(oRes.start) !== 'undefined' ) {
							var filterPattern = /([^ ]*) \\(([^\\)]*)\\)/;
							var sAnnotMatch = oRes.annot.match(filterPattern);
							var sMatchAcc = sAnnotMatch[1];							
							if ( oMet.id === "functional_residue") {
								var sMatchId = sMatchAcc;
								sAnnot = "<a href='"+sLigandURL+sMatchId+"' target='_blank'>"+sMatchAcc+"</a>";
							}
							if ( residue == oRes.start ) {
								res += "<tr><td colspan='2'>"+oRes.start+"</td><td>"+sAnnot+"</td>";
							}
						}
					}
					if ( res !== '' ) {
						met += "<tr><td colspan='4'class=''><h5>"+oMet.label+"</h5></td></tr>";
						met += "<tr><td>start</td><td>end</td><td>"+oMet.title+"</td></tr>";
						met += res;
					}
				}
			}
			if ( met !== '' ) {
				annot += "<table border=1 class='table table-bordered table-condensed'>"+met+"</table>";
			}				
			break;
		}
	}
...
/* appris */
seq-viewer.seq-browser-label.a, .seq-browser-label.a label, .seq-browser-label.a > span.glyphicon.glyphicon-check { /* legend */
	color: rgba(180,95,4,1);
}
/* firestar */
.seq-browser-label.f > span.glyphicon.glyphicon-check { /* icon legend */
	color: rgba(244,169,39,1);
}
.seq-browser-label.f label { /* legend */
	color: rgba(244,169,39,1);
	text-decoration: underline;
}
seq-viewer.seq-browser-label.f { /* seq-viewer */
	color: rgba(244,169,39,1) !important;
	text-decoration: underline;
}
/* matador3d */
seq-viewer.seq-browser-label.m, .seq-browser-label.m label, .seq-browser-label.m > span.glyphicon.glyphicon-check { /* legend */
	color: #800000;
}
seq-viewer.seq-browser-label.m { /* seq-viewer */
	font-weight: bold;
}
/* spade */
.seq-browser-label.s > span.glyphicon.glyphicon-check {
	color: rgba(164,189,83,1);
}
.seq-browser-label.s label {
	background-color: rgba(164,189,83,1);
	color: #ffffff;	
}
seq-viewer.seq-browser-label.s { /* seq-viewer */
	background-color: rgba(164,189,83,1);
}
/* corsair */
.seq-browser-label.c > span.glyphicon.glyphicon-check { /* icon legend */
	color: rgba(4,95,180,1);
}
.seq-browser-label.c label { /* legend */
	background-color: rgba(4,95,180,1);
	color: #ffffff;
}
seq-viewer.seq-browser-label.c { /* seq-viewer */
	background-color: rgba(4,95,180,1);
}
/* thump */
.seq-browser-label.t > span.glyphicon.glyphicon-check { /* icon legend */
	color: rgba(245,169,208,1);
}
.seq-browser-label.t label { /* legend */
	background-color: rgba(245,169,208,1);
	color: #ffffff;
}
seq-viewer.seq-browser-label.t { /* seq-viewer */
	background-color: rgba(245,169,208,1);
}
/* crash */
.seq-browser-label.cr > span.glyphicon.glyphicon-check { /* icon legend */
	color: rgba(153,153,102,1);
}
.seq-browser-label.cr label { /* legend */
	background-color: rgba(153,153,102,1);
	color: #ffffff;
}
seq-viewer.seq-browser-label.cr { /* seq-viewer */
	background-color: rgba(153,153,102,1);
}
</style>
</head>
...
$HTML_TEMPLATE2 = <<HTML_TEMPLATE2;
<script>
\$( "seq-viewer" ).webuiPopover( {
	trigger: 'click',
	closeable:true,
	multi:false,	
	type:'html',
	animation: 'pop',
	width: '400',
	title: function(data, x) {
		return "APPRIS annotations";
	},
	content: function() {
		var sid = \$(this).attr('id');
		var methods = \$(this).attr('methods');
		var residue = \$(this).attr('residue');
		return printAnnotSeqViewer(sid,methods,residue);
	},
});
</script>
HTML_TEMPLATE2
