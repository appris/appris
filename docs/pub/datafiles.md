...
* Transcript Support Level (TSL):
	...
	The following categories are assigned to each of the evaluated annotations:
		tsl1 Ð all splice junctions of the transcript are supported by at least one non-suspect mRNA
		tsl2 Ð the best supporting mRNA is flagged as suspect or the support is from multiple ESTs
		tsl3 Ð the only support is from a single EST
		tsl4 Ð the best supporting EST is flagged as suspect
		tsl5 Ð no single transcript supports the model structure
		tslNA Ð the transcript was not analysed for one of the following reasons:
