...
def load_refseq_metadata(annot_file, transl_file):
    
    transl_seq_meta = SlottedDict()
    with open_as_text(transl_file) as f:
        for header, sequence in SimpleFastaParser(f):
            match = refseq_transl_hdr_regex.match(header)

            transl_id_ver = match['ens_transl_id']
            transl_id = transl_id_ver
            transl_id = re.sub(r'^.*\|ref\|','', transl_id)
            transl_id = re.sub(r'\|.*$','', transl_id)
            transl_id = re.sub(r'\..*$','', transl_id)

            transl_seq_meta[transl_id] = {
                'translation_seq_sha1': calc_sha1(sequence),
                'length': len(sequence)
            }
    ...
    import sys
    sys.path.append("/local/appris/scripts/lib")
    import GTF
    annot_df = GTF.dataframe(annot_file)

    annot_df = annot_df.loc[(annot_df['feature'] == 'CDS')]

    gene_to_isoforms = defaultdict(set)
    for dbxref, transc_id_ver, transl_id_ver in zip(annot_df['Dbxref'],
                                                         annot_df['Parent'],
                                                         annot_df['Name']):
        dbxref_2 = dbxref if isinstance(dbxref, str) else "|".join(dbxref)
        gene_to_isoforms[dbxref_2].add((transc_id_ver, transl_id_ver))

    isoform_meta = SlottedDict()
    for dbxref, isoforms in gene_to_isoforms.items():
        
        match = rs_gene_id_regex.match(dbxref)
        gene_id = match['bare_id']

        for transc_id_ver, transl_id_ver in isoforms:
            if transl_id_ver is not  None:
                transc_id = transc_id_ver
                transc_id = re.sub(r'^[^\-]*\-','', transc_id)
                try:
                    match = ens_id_regex.match(transc_id)
                    transc_id = match['bare_id']
                    transc_version = match['id_version']
                except:
                    transc_id = re.sub(r'\..*$','', transc_id)
                ...
                transl_id = re.sub(r'^[^\-]*\-','', transl_id)
                # transl_id = re.sub(r'\..*$','', transl_id)
                match = ens_id_regex.match(transl_id)
                transl_id = match['bare_id']
                transl_version = match['id_version']


                transl_seq_sha1 = transl_seq_meta[transl_id]['translation_seq_sha1']
                aa_length = int(transl_seq_meta[transl_id]['length'])

                isoform_key = (transl_id, aa_length)
                if isoform_key not in isoform_meta:
                    isoform_meta[isoform_key] = {
                        'gene_id': gene_id,
                        'transcript_id': transc_id,
                        'transcript_version': transc_version,
...
rs_gene_id_regex = re.compile('^.*GeneID:(?P<bare_id>[^\|]+).*$') # 'GeneID:9651|Genbank:XP_016858362.1|HGNC:HGNC:29037|MIM:612836'
...
refseq_transl_hdr_regex = re.compile(
    '^(?P<ens_transl_id>\S+)'
)
...
with TemporaryDirectory() as tmp_dir:
    ...
    with open_as_text(pred_file_path) as f:
        ...
        for rec in reader:
            ...
            transc_key = (transl_id, aa_length)
            meta_rec = isoform_meta[transc_key]

            for k in ('gene_id', 'transcript_id'):
                rec[k] = rec[k]
            for k in ('transcript_version','translation_id', 'translation_version', 'translation_seq_sha1', 'length'):
                rec[k] = meta_rec[k]
