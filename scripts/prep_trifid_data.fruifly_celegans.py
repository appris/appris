...
def load_ensembl_metadata(annot_file, transl_file):
    ...
    with open_as_text(transl_file) as f:
        for header, sequence in SimpleFastaParser(f):
            ...
            transl_key = match['ens_transl_id']
    ...
    rel_cols = ['gene_id', 'transcript_id', 'protein_id']
    ...
    for _,row in annot_df.iterrows():
        ...
        transc_key = row['transcript_id']
        transl_key = row['protein_id']
    ...
    for gene_key, isoforms in gene_to_isoforms.items():
        ...
        for transc_key, transl_key in isoforms:
            transc_id = transc_key
            transl_id = transl_key
...
with TemporaryDirectory() as tmp_dir:
    ...
    with open_as_text(pred_file_path) as f:
        ...
        for rec in reader:
            ...
            for k in ('gene_id', 'transcript_id', 'translation_id', 'translation_seq_sha1', 'length'):
                rec[k] = meta_rec[k]
