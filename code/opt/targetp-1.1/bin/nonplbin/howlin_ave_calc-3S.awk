#
# FILE howlin_ave_calc-3S.awk
# USAGE paste howlin.result.$$.? | nawk -f howlin_ave_calc-3S.awk -v noofseq=$antal -v Tcut=# -v Scut=# -v Ocut=#
#
# Calculate average over the 5 different NN for all 3 categories. 
...
END {
# performing the prediction (winner-takes-all, with or without cutoff restrictions)
	for (i=1; i<=noofseq; i++) { 
		pred_score[i]=0.000
		pred[i]="?"	
		for (j=1; j<=no_cat; j++) {
...
# presentation of results
	for (i=1; i<=noofseq; i++) {
	printf "%5.3f %5.3f %5.3f %3s     %1d\n",score[(i-1)*no_cat+1], \
		score[(i-1)*no_cat+2],score[(i-1)*no_cat+3], pred[i], \
		rel_class[i] 
	}
}
