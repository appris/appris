# FILE analys2data_A-4S.awk
#
# IN:	files with averaged HOW network output ([cTP/mTP/SS]5to1HOW_scores.$$)
...
/^ #/ {
	...
	while  (i<len) {
		getline
		i++
		if (i<=w) printf "%5.3f  %5.3f  %5.3f  ", $5, $10, $15			
	}
	if (len<w) {
		for (j=1; j<=w-len; j++) {		  # om <100 fylls de på
			if (i+j<=w) printf "0.000  0.000  0.000  "
		}
	}	
	printf "%2.0f %2.0f %2.0f %2.0f \n",cTP_transit,mTP_transit,SS_transit,no_transit
	getline		# to get it right with the [cTP/mTP/SS]5to1HOW_scores.$$-file
	next
}
