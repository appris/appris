...
# Required arguments
unless( defined $chr and defined $target and 
		defined $gff_file and defined $transl_file and
		defined $maf_file and defined $work_dir)
{
	print `perldoc $0`;
	exit 1;
}
...
sub main()
{
	...
	$work_dir = prepare_workspace($work_dir);
	$logger->error("ERROR: creating workspace ") unless (defined $work_dir);
	
	# gff parsing
	$logger->info("-- get cds coords for transc\n");
	my ($transcript_cds_list, $cds_list, $strand_cds_list) = get_cds_list_for_transcript($chr, $gff_file, $transl_file, $work_dir);
	...
	my ($transcrip_multi_alignment) = create_cds_alignments($chr, $transcript_cds_list, $cds_2_sequences, $cds_2_organims, $work_dir);
...
This program took the information about exon position and coordinates, 
read a maf alignment file and extract all the exons and the sequences aligned.
...
=head2 Required arguments:
	...
	--chr <Human genomic coordinate>
	
	--target <Target sequence. hg19 -human->
			
	--maf <Maf file>

	--gff <GFF2 file>
	
	--transl_file  <Gencode translations file>
	
	--output_dir <Dir where will save the cds alignments>
...
=head1 EXAMPLE
	...
	perl getCDSAlignment.pl
	
		--chr=21
		
		--target=hg19

		--maf=/local/jmrodriguez/projects/Encode/MAF_alignments/hg19/chr21.maf
		
		--gff=../features/gencode.v7.annotation.gtf
		
		--transl_file=../features/gencode.v7.pc_translations.fa
		
		--output_dir=../features/sequences/chr21/cds_alignments
