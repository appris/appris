...
/*  $Id: showsum.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 28 $  */
...
#else
#define BSFNUM(i) bptr[i]->seq->sfnum
...
lnum_sort(struct beststr **s, int n)
{
  ...
  for (i=0; i<n-1; i++)
    if (s[i]->seq->lseek > s[i+1]->seq->lseek) goto l2;
  return;
...
l2:
  for (gap=n/2; gap>0; gap/=2)
    for (i=gap; i<n; i++)
      for (j=i-gap; j>=0; j -= gap) {
	if (s[j]->seq->lseek <= s[j+gap]->seq->lseek) break;
	btmp = s[j];
	s[j]=s[j+gap];
	s[j+gap]=btmp;
      }
