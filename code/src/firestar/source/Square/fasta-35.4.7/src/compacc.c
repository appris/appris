...
/*  $Id: compacc.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 46 $  */
...
int
reset_maxn(struct mngmsg *m_msp, int maxn) {

  /* reduce maxn if requested */
  if (m_msp->maxn > 0 && m_msp->maxn < maxn) maxn = m_msp->maxn;

  if (m_msp->qdnaseq==m_msp->ldnaseq || m_msp->qdnaseq==SEQT_DNA ||
   ...
   if (m_msp->n0> m_msp->max_tot/3) {
    ...
    m_msp->l_overlap = m_msp->n0;
    m_msp->maxt3 = maxn-m_msp->l_overlap;
  }
  else {	/* is TFAST */
    if (m_msp->n0 > MAXTST) {
    ...
    m_msp->l_overlap = 3*m_msp->n0;
    m_msp->maxt3 = maxn-m_msp->l_overlap-3;
    m_msp->maxt3 -= m_msp->maxt3%3;
    m_msp->maxt3++;
...
void
do_stage2(struct beststr **bptr, int nbest, const struct mngmsg *m_msp0,
  ...
  char *seqc_buff, *seqc, *annc_buff, *annc;
  int seqc_buff_len, aln_code_n, annc_buff_len, ann_code_n;
#ifdef MPI_SRC
  ...
  while (node_done < nnodes-FIRSTNODE) {
    ...
    seqc_buff = NULL;
    annc_buff = NULL;
    if (s_func == DO_OPT_FLG && m_msp0->show_code==SHOW_CODE_ALIGN) {
#ifdef PVM_SRC
      bufid = pvm_recv(tid,CODERTYPE);
      pvm_upkint(&seqc_buff_len,1,1);	/* get the code string length */
      pvm_upkint(&annc_buff_len,1,1);	/* get the code string length */
      pvm_freebuf(bufid);
#ifdef DEBUG
      fprintf(stderr," Have code buf counts: %d %d\n",seqc_buff_len, annc_buff_len);
#endif
#endif
#ifdef MPI_SRC
      MPI_Recv((char *)&seqc_buff_len,1,MPI_INT, snode,
	       CODERTYPE,MPI_COMM_WORLD, &mpi_status);
      MPI_Recv((char *)&annc_buff_len,1,MPI_INT, snode,
	       CODERTYPE,MPI_COMM_WORLD, &mpi_status);
#endif

      /* get seqc_buff - alignment encoding */
      seqc=seqc_buff = NULL;
...
#ifdef DEBUG
	  fprintf(stderr," waiting for code data\n");
#endif
#ifdef PVM_SRC
	  bufid = pvm_recv(tid,CODERTYPE2);
	  pvm_upkbyte(seqc_buff,seqc_buff_len*sizeof(char),1);
#endif
#ifdef MPI_SRC
	  MPI_Recv((char *)seqc_buff,seqc_buff_len*sizeof(char),
		   MPI_BYTE,snode,CODERTYPE2,MPI_COMM_WORLD, &mpi_status);
#endif
	}
      }

      /* get annc_buff - alignment encoding */
      annc_buff = NULL;
      if (annc_buff_len > 0) {		/* allocate space for it */
	if ((annc=annc_buff=calloc(annc_buff_len,sizeof(char)))==NULL) {
	  fprintf(stderr,"Cannot allocate annc_buff: %d\n",annc_buff_len);
	  annc_buff_len=0;
	}
	else {
#ifdef PVM_SRC
	  pvm_upkbyte(annc_buff,annc_buff_len*sizeof(char),1);
#endif
#ifdef MPI_SRC
	  MPI_Recv((char *)annc_buff,annc_buff_len*sizeof(char),
		   MPI_BYTE,snode,CODERTYPE2,MPI_COMM_WORLD, &mpi_status);
#endif
	}
      }
	...
	if (s_func == DO_CALC_FLG && m_msp0->last_calc_flg) {
	...
	else if (m_msp0->stages > 1) {
	  bbptr->rst.score[0] = bestr2[i].rst.score[0];
	  bbptr->rst.score[1] = bestr2[i].rst.score[1];
	  bbptr->rst.score[2] = bestr2[i].rst.score[2];
	}

	if (s_func == DO_OPT_FLG && m_msp0->markx & MX_M9SUMM) {
	  ...
	  if (m_msp0->show_code == 2) {	/* if show code */
	    /* length of encoding */
	    aln_code_n = bbptr->aln_code_n = bestr2[i].aln_code_n;
	    ann_code_n = bbptr->ann_code_n = bestr2[i].ann_code_n;
	    bbptr->aln_code = NULL;
	    bbptr->ann_code = NULL;
	    ...
	    if (ann_code_n > 0) {
	      if ((bbptr->ann_code = 
		   (char *)calloc(ann_code_n+1,sizeof(char)))==NULL) {
		fprintf(stderr,"cannot allocate ann_code[%d:%d]: %d\n",
			bbptr->seq->wrkr,bbptr->seq->seqnm,ann_code_n);
		annc += ann_code_n+1;
		bbptr->ann_code_n = 0;
	      }
	      else {
		strncpy(bbptr->ann_code,annc,ann_code_n);
		bbptr->ann_code[ann_code_n]='\0';
		annc += ann_code_n+1;
	      }
	    }
	  }
