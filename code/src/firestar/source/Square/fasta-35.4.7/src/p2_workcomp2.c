...
/*  $Id: p2_workcomp2.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 28 $  */
...
extern void initseq(char **seqc0, char **seqc1, char **seqca, int seqsiz);
extern void initseq_arr(char **, char **, int);

extern void freeseq(char **seqc0, char **seqc1, char **seqca);
extern void freeseq_ann(char **, char **);

void send_bestr(int, int, struct comstr *, int, int);
void send_bestr2(int, struct comstr2 *, int);
void send_struct(int hosttid, int msg_type, void *void_p, size_t slen);
void send_code(int, char *, int, char *, int);
...
#endif
{
  ...
  struct mngmsg m_msg;	/* start message from manager to worker 1 -- only sent once */
  ...
  char info_gstring2[2][MAX_STR];		/* parameter string for manager */
  char *info_gstring2p[2] = {info_gstring2[0], info_gstring2[1]};		/* parameter string for manager */
  char info_gstring3[MAX_STR];		/* parameter string for manager */
  ...
  char lib_range[MAX_SSTR];
  char libstr[21];
  char errstr[128];
  int itt=0;
  int bufid;
  char *seqc0, *seqc1, *seqca;	/* consensus alignment */
  char *seqc0a, *seqc1a;	/* buffers for annotations */
  char *seqc, *seqc_buff, *seq_code;
  int seqc_buff_cnt, seqc_buff_len, seqc_max;
  char *annc, *annc_buff, *ann_code;
  int annc_buff_cnt, annc_buff_len, annc_max;
  int maxc, lc, nc, nident, ngap, aln_code_n, ann_code_n;
...
/*****************************************************************/
      ...
      cur_n0 = qm_msg.n0;
      if (m_msg.ann_flg && qm_msg.have_ann) {
	old_have_ann = 1;
...
/*****************************************************************/
    ...
    /* s_func == 1 means do_opt if necessary */
      ...
      seqc_buff = annc_buff = NULL;
      seqc_buff_len = seqc_buff_cnt = annc_buff_len = annc_buff_cnt = 0;
	...
	if (m_msg.ann_flg && qm_msg.have_ann) {
	  annc_buff_len = (BFR2+5)*1024;
	  annc = annc_buff = (char *)calloc(annc_buff_len,sizeof(char));
	  if (annc_buff == NULL) {annc_buff_len = 0;}
	  annc_buff_cnt = 0;
	}
      }
	...
	if (m_msg.markx & MX_M9SUMM) {
	  ...
	  if (m_msg.show_code == SHOW_CODE_ALIGN) {
	    ...
	    cal_coord(qm_msg.n0,seqpt[seqnm].n1,
		      m_msg.q_offset + (m_msg.q_off-1) + (m_msg.sq0off-1),
		      m_msg.sq1off-1,
		      aln_dp);

	    seqc_max = seqpt[seqnm].a_res[itt].nres + 4*aln_dp->llen+4;
	    if (seqc_max > seqc_buff_len-seqc_buff_cnt-256) {
	      seqc_max = max(0,seqc_buff_len-seqc_buff_cnt-256);
	    }
	    if (seqc_max > 0) {seq_code = seqc;}
	    else {seq_code = NULL;}

	    ann_code = NULL;
	    annc_max = 0;
	    if (m_msg.ann_arr[0] != '\0') {
	      annc_max = 4*seqc_max;
	      if (annc_max > annc_buff_len-annc_buff_cnt-512) {
		annc_max = max(0,annc_buff_len-annc_buff_cnt-512);
	      }
	      if (annc_max > 0) {ann_code = annc;}
	    }

	    if (seqc_max > 256) {
	      lc=calc_code(aa0[itt],qm_msg.n0,
			   seqpt[seqnm].aa1, seqpt[seqnm].n1,
			   aln_dp,&seqpt[seqnm].a_res[itt],
			   &pst, seq_code, seqc_max,
			   m_msg.ann_arr, m_msg.aa0a, NULL, ann_code, annc_max,
			   f_str[itt]);

	      aln_code_n = strlen(seq_code);
	      seqc += aln_code_n;
	      *seqc++ = '\0';
	      ...
	      ann_code_n = 0;
	      if (ann_code != NULL) {
		ann_code_n = strlen(ann_code);
		annc += ann_code_n;
		*annc++ = '\0';
		annc_buff_cnt += ann_code_n+1;
	      }
/*
...
*/
	  ...
	  bestr2[bestcnt].ann_code_n = ann_code_n;
	}
	bestcnt++;
	...
	if (bestcnt >= BFR2) {
	  send_bestr2(hosttid,bestr2,bestcnt);
	  if (m_msg.show_code == SHOW_CODE_ALIGN) {
	    send_code(hosttid,seqc_buff,seqc_buff_cnt,annc_buff, annc_buff_cnt);
	    memset(seqc_buff,0,seqc_buff_cnt);
	    seqc = seqc_buff;
	    seqc_buff_cnt = 0;
	    if (annc_buff != NULL) memset(annc_buff,0,annc_buff_cnt);
	    annc = annc_buff;
	    annc_buff_cnt = 0;
	  }
	  bestcnt = 0;
...
*/
	...
	*/
	...
	if (m_msg.ann_flg && qm_msg.have_ann) {
	  pvm_pkbyte(seqc0a,maxc,1);
	  pvm_pkbyte(seqc1a,maxc,1);
	}
...
/*	p4_dprintf("[%d] sending aln\n",worker); */
	...
	if (m_msg.ann_flg && qm_msg.have_ann) {
	  MPI_Send(seqc0a,maxc,MPI_BYTE,hosttid,ALN2TYPE,MPI_COMM_WORLD);
	  MPI_Send(seqc1a,maxc,MPI_BYTE,hosttid,ALN2TYPE,MPI_COMM_WORLD);
	}
#endif
...
void
send_code(int hosttid, 
	  char *seqc_buff, int seqc_buff_cnt,
	  char *annc_buff, int annc_buff_cnt) {

#ifdef DEBUG
  fprintf(stderr," sending seqc_buff_cnt: %d, annc_buff_cnt: %d\n",seqc_buff_cnt, annc_buff_cnt);
#endif
#ifdef PVM_SRC
  /* send info */
  pvm_initsend(PvmDataRaw);
  pvm_pkint(&seqc_buff_cnt,1,1);
  pvm_pkint(&annc_buff_cnt,1,1);
  pvm_send(hosttid,CODERTYPE);
  /* send data */
  if (seqc_buff_cnt > 0) {
#ifdef DEBUG
    fprintf(stderr," sending seqc data:\n");
#endif
    pvm_initsend(PvmDataRaw);
    pvm_pkbyte(seqc_buff,seqc_buff_cnt,1);
    if (annc_buff_cnt > 0) pvm_pkbyte(annc_buff,annc_buff_cnt,1);
    pvm_send(hosttid,CODERTYPE2);
  }
#endif
#ifdef MPI_SRC
  MPI_Send(&seqc_buff_cnt,1,MPI_INT,
	   hosttid,CODERTYPE,MPI_COMM_WORLD);
  MPI_Send(&annc_buff_cnt,1,MPI_INT,
	   hosttid,CODERTYPE,MPI_COMM_WORLD);
  if (seqc_buff_cnt>0) MPI_Send(seqc_buff,seqc_buff_cnt,MPI_BYTE,
			       hosttid,CODERTYPE2,MPI_COMM_WORLD);
  if (annc_buff_cnt>0) MPI_Send(annc_buff,annc_buff_cnt,MPI_BYTE,
			       hosttid,CODERTYPE2,MPI_COMM_WORLD);
#endif
}
