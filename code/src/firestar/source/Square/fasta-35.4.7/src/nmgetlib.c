...
/*  $Id: nmgetlib.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 104 $  */
...
{
  char rline[10],sname[MAX_FN], iname[MAX_FN];
  char *bp, *bp1, *bp2;
 ...
 l1:
  ...
  else if (libtype==ACC_LIST) {
    /* open the file, read the first line, do an openlib on the first
       line - could openlib be recursive?? */
    if (!use_stdin) {
      opnflg=((libf=fopen(sname,RBSTR))!=NULL);
    }
    else {
  ...
  if (libtype <= LASTTXT) {
    m_fptr->lpos = 0;
    /* if I cannot read a line, file is empty, and lline must be loaded before first getseq() */
