...
/*  $Id: dropfz2.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 83 $  */
	   ...
	   int *have_ares)
{
  ...
  a_res->res = f_str->res;	/* pro_dna needs a valid res at start */
  *have_ares = 1;	/* pre_cons() required before calcons() */
  a_res->min0 = a_res->min1 = a_res->max0 = a_res->max1 = 0;

#ifndef TFAST
  /* everything is already pre-computed for the query */
...
#else
  /* make a precomputed codon number series for the library sequence */
  if (frame==0) {
    pre_com(aa1, n1, f_str->aa1v);
  }
  ...
  /* but we do not need translations, because we're not running do_fasta */
  /* when we start using a band, rather than a all-seq vs all-seq
     alignment, then we will need do_fasta to find a region to limit to */
  ...
  while (rp < rpmax ) {
	...
	*sp0a++ = ' ';
      ...
      *sp0 = f_str->weight_c[aap][ap0[i0]].c5;
      ...
      if ((itmp=ppst->pam2[0][aap][pascii[*sp0]])<0) { *spa = M_NEG; }
...
/* now get the middle */
  ...
  while (rp < rpmax ) {
    ...
    case 3:			/* match */
      ...
      sp0 = f_str->weight_c[aap][ap0[i0]].c5;
      if (ppst->pam2[0][aap][pascii[sp0]]>=0) {
	sim_code = M_POS;
	aln->nsim++;
      }
	...
	aln->nident++;
      ...
      if (have_ann && ann_arr[aa0a[i0]] != ' ') {
	sprintf(tmp_astr, "|%ld:%ld:%c%c%c%c",
		aln->q_offset+i1+1,aln->l_offset+i0+1,ann_arr[aa0a[i0]],sim_sym[sim_code],sp0,sp1);
    ...
    case 2:			/* -1 frame shift */
      ...
      sp1 = ppst->sq[aap=ap1[i1++]];
      sp0 = f_str->weight_c[aap][ap0[i0]].c2;
      if (ppst->pam2[0][aap][pascii[sp0]]>=0) { aln->nsim++; }
