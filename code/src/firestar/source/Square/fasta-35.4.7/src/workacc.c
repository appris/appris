...
/*  $Id: workacc.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 28 $  */
...
qshuffle(unsigned char *aa0, int n0, int nm0)
{
  ...
  for (k=1; k<nm0; k++) {
/*  aap = aa0start[k];
    while (*aap) fputc(ppst->sq[*aap++],stderr);
    fputc('\n',stderr);
*/
    aa0start[k][-1]=ESS;
  }
...
void initseq(char **seqc0, char **seqc1, char **seqca, int seqsiz)	/* initialize arrays */
{
  *seqc0=(char *)calloc((size_t)(seqsiz+1)*4,sizeof(char));
  *seqc1= *seqc0+seqsiz+1;
