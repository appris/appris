...
/*  $Id: scaleswn.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 93 $  */
	     ...
	     int do_hist)
{
  ...
  if (m_msg->n0 <= 2*LENGTH_CUTOFF && (zsflag == REG_STATS || zsflag == REGI_STATS || zsflag == REG2_STATS)) {
    zsflag = MLE_STATS;
  }
...
*/
  ...
  if (zsflag==LN_STATS) {
    find_zp = &find_zl;
    ps_s->zsflag=r_zsflag = proc_hist_ln(sptr, nstats, histp, do_trim, ps_s);
  }
#else
  if (zsflag==MLE_STATS) {
    find_zp = &find_ze;
    ps_s->zsflag=r_zsflag = proc_hist_ml(sptr, nstats, ppst, histp, do_trim, ps_s);
  }
#endif
  else if (zsflag==REG_STATS) {
    find_zp = &find_zr;
    ps_s->zsflag=r_zsflag = proc_hist_r(sptr, nstats, ppst, histp, do_trim,  ps_s);
  }
  else if (zsflag==AG_STATS) {
    find_zp = &find_za;
    ps_s->zsflag=r_zsflag = proc_hist_a(sptr, nstats, ppst, histp, do_trim,  ps_s);
  }
  else if (zsflag==REGI_STATS) {
    find_zp = &find_zr;
    ps_s->zsflag=r_zsflag = proc_hist_r2(sptr,nstats, ppst, histp, do_trim,  ps_s);
  }
    ...
    find_zp = &find_ze2;
    ps_s->zsflag=r_zsflag = proc_hist_ml2(sptr, nstats, ppst, histp, do_trim,  ps_s);
  }
#endif
  else {	/* AVE_STATS */
    find_zp = &find_zn;
    ps_s->zsflag=r_zsflag = proc_hist_n(sptr,nstats, ppst, histp, do_trim,  ps_s);
  }

  if (!ppst->zdb_size_set) ppst->zdb_size = m_msg->db.entries;
	    ...
	    int do_trim, struct pstat_str *pu)
{
  ...
  if (!llen.fit_flag) {	/* the fit failed, fall back to proc_hist_ml */
    free_hist(&llen);
    find_zp = &find_ze;
    return proc_hist_ml(sptr,nstats, ppst, histp, do_trim, pu);
  }
	    ...
	    int do_trim, struct pstat_str *pu)
{
  ...
  else if (strcmp(ppst->pamfile,"MD10")==0)
      r_v = look_p(md10_p,t_gdelval,t_ggapval,&K,&Lambda,&H);
  else if (strcmp(ppst->pamfile,"MD20")==0)
      r_v = look_p(md20_p,t_gdelval,t_ggapval,&K,&Lambda,&H);
  else if (strcmp(ppst->pamfile,"MD40")==0)
...
E1_to_s(double e_val, int n0, int n1, int db_size,
  ...
  if (n0 < LENGTH_CUTOFF || n1 < LENGTH_CUTOFF) return 0;
...
s_to_bit(int score, int n0, int  n1, struct pstat_str *pu) { 
  ...
  double bit;
  double mp, np, a_n0, a_n1;
  double z, log_len, p_val;
	     ...
	     struct pstruct *ppst, struct pstat_str *rs)
{
  ...
  if (ppst->zsflag < 0 || ppst->zsflag_f < 0) {
    ...
    for (i=0; i<nbest; i++) {
      zscore = find_zp(bptr[i]->rst.score[ppst->score_ix], bptr[i]->rst.escore,
