...
/*  $Id: apam.c 27 2008-06-30 16:27:31Z pearson $ */
/* $Revision: 94 $  */
...
struct std_pam_str std_pams[] = {
  ...
  {"VT160","VT160", avt160,  0.231049, -12, -2},
  {"OPT5","OPTIMA5", aopt5,  0.138629, -20, -2},
  {"\0",   "\0",     NULL,   0.0,        0,  0}
};
...
/* init_ascii()
   ...
   also ensures that no letters are outside range
*/
void
init_ascii(int is_ext, int *sascii, int pnsq, int is_dna) {
  ...
  /* this code guantees that all characters in sq are represented in
     sascii[], but it does not guarantee that everything else in
     sascii[], particularly sascii[O,U,J], have appropriate values
 */
  have_lc = 0;
  ...
  /* these checks need to be done after sascii[] has been
     re-initialized */
  if (is_protein) {
    if (sascii['O'] > pnsq) { sascii['O'] = sascii['K']; sascii['o'] = sascii['k'];}
    if (sascii['U'] > pnsq) { sascii['U'] = sascii['C']; sascii['u'] = sascii['c'];}
    if (sascii['J'] > pnsq) { sascii['J'] = sascii['L']; sascii['j'] = sascii['l'];}
  }

  /* one final check for characters out of range (> nsq)*/
  err_pnsq_limit = 0;
  for (isq = 'A'; isq <= 'Z'; isq++) {
    if (sascii[isq] < NA && sascii[isq] > pnsq) {
      fprintf(stderr, " *** ERROR *** sascii['%c']:%d > %d\n",isq, sascii[isq], pnsq);
      err_pnsq_limit = 1;
    }
  }
  if (err_pnsq_limit) {exit(1);}
