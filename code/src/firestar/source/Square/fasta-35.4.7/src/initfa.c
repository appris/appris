...
/*  $Id: initfa.c 27 2008-06-30 16:27:31Z pearson $ */
/*  $Rev: 116 $  */
...
char *verstr="version 35.04 Mar. 26, 2009";
 ...
 *  currently, this function seems very protein centric
 */
void
	  ...
	  struct mngmsg *m_msg, struct pstruct *ppst)
{
  ...
  case 's':
	...
	strncpy(ppst->pamfile,pgm_def_arr[pgm_id].smstr,MAX_FN);
      ...
      /* all the scoring matrices provided are protein */
      ppst->dnaseq = SEQT_PROT;
    }
    ppst->pam_set=1;
    break;
...
#ifndef TFAST
#ifdef DNALIB_LC
    init_ascii(ppst->ext_sq_set,lascii,ppst->nsq, m_msg->ldnaseq);
#endif
#else
