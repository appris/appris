...
$RELEASE				= $cfg->val('FIRESTAR_VARS', 'release');
...
$FILTER_LIST			= $cfg->val('FIRESTAR_VARS', 'provisional_filter');
...
sub main()
{
	...
	# Get Gene ID
	my @id_array = split("/", $input_file);
	my $gene_id = $id_array[-3]; # Es la posición -3 porque appris mete una / de más en los paths y deja un hueco vacío

	# Check if the Gene_id is in filter_list
	open(F,$FILTER_LIST);
	my @filter_array=<F>;close F;
	chomp(@filter_array);
	
	my @filtered=grep /$gene_id/,@filter_array;
	my $filter_gene = "";
	...
	if ($filter_gene eq "False") {
		...
		foreach my $varname (keys %gene_vars)
		{
			...
			my ($firePredText_file) = $ws_cache.'/'.'seq.firestar_'.$RELEASE;
	...
	#		}
		...
		$logger->info("\nGene ID $gene_id has been discarded for firestar.\n");
