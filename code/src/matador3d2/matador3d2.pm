...
#				close FH;
			unless(-e $output_ali_file and (-s $output_ali_file > 0) and -e $output_dom_file_sorted and (-s $output_dom_file_sorted > 0) and ($main::CACHE_FLAG eq 'yes')) # Cached Blast
			{
				...
				`$hmmscan_bin --cpu $num_cpus -o $output_ali_file --notextw --domtblout $output_dom_file $database $sequence_file`;
...
#		my $hmmscan_file = "$tmp_dir/$transcript_id/matador3D.domtblout.sorted";
		...
		while(my $line = <FH>){
			...
			my $raw_bitscore 		= $bitscore;
			my $derived_bitscore 	= $bitscore;
			if($reduction_factor){
				if($raw_bitscore >= $reduction_factor){
					$derived_bitscore = $bitscore;
				}
				else{
					my $reduction 		= 1 - ($reduction_factor-$raw_bitscore)/$reduction_factor;
					$derived_bitscore 	= sprintf("%.2f", $reduction * $raw_bitscore);
				}
			}
			
			if($derived_bitscore < $min_bitscore){
				next;
			}
			
			$domains->{$template}{$transcript_id}{$start}{$end}{bitscore} 		= $derived_bitscore;
			$domains->{$template}{$transcript_id}{$start}{$end}{raw_bitscore} 	= $raw_bitscore;
			$domains->{$template}{$transcript_id}{$start}{$end}{bias} 			= $bias;
			$domains->{$template}{$transcript_id}{$start}{$end}{num_domains} 	= $num_domains;
			
			$domains_sortable_list->{"$template|$transcript_id|$start|$end"} 	= $derived_bitscore;
		}
		close FH;
	}
}
...
#			}
			if($num_subseqs > 1){
				...
				if(!$overlap){
					...
					$domains->{$template}{$other_isoform}{$other_iso_start}{$other_iso_end}{raw_bitscore} 	= $domains->{$template}{$isoform}{$start}{$end}{raw_bitscore};
	...
	foreach my $isoform (keys %$gene){
		...
		foreach my $domain_to_transfer (@{$gene->{$max_isoform}{domains}}){
			...
			if(scalar @sub_seqs > 1){
				...
				my $raw_bitscore 	= $domains->{$template}{$max_isoform}{$start}{$end}{raw_bitscore};
				...
				$domains->{$template}{$isoform}{$other_iso_start}{$other_iso_end}{raw_bitscore} = $raw_bitscore;
			}
			else{
...
#		print "$gene_id\t$isoform\t$global_score";
		...
		foreach my $domain (@{$gene->{$isoform}{domains}}){
			...
			my $raw_bitscore 	= $domains->{$template}{$isoform}{$start}{$end}{raw_bitscore};
			
		 	#ÊBEGIN: CHANGE for APPRIS
#			print "\t$template;$bitscore;$bias;$start-$end;" . ($end - $start + 1) . ";$raw_bitscore";
...
sub usage {
	...
	print STDERR "Usage: \nmatador3D2.reduction_factor.pl -i input_fasta_file -bin hmmscan_bin -db database [-t temp_dir] [-rf reduction_factor]\n";
	...
	print STDERR "-rf  -> Reduction factor, for each hit with a raw bitscore lower than this value, a (linear) reduction factor is applied to the raw bitscore. This reduction is higher as the raw bitscore is lower (it decreases linearly from 1 to 0, from bitscore = 0 up to bitscore = reduction_factor), effectively reducing the influence of low or relative low bitscores. Optional, default is 50. If 0, the reduction is not applied.\n";
	print STDERR "-min -> Minimum bitscore, all hits with a bitscore lower than this value is discarded. Optional, default is 1.\n";
