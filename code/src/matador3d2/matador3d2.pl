...
sub main()
{
	...
	my $sequences = matador3d2::read_fasta_file($input_file);

	# Run each hmmscan for each sequence and store results in temporary directory
	$logger->info("-- run hmmscan\n");
	matador3d2::run_hmmscan($RUN_PROGRAM, $sequences, $WSPACE_CACHE, $PROG_DB_DIR);
	
	# Score isoforms
	$logger->info("-- score isoforms\n");
	my $scoring = matador3d2::run_scoring($sequences, $WSPACE_CACHE);
